...
11. RESSOURCES
12. PIPELINE DES APPELS DE FONCTIONS
...
priority: "express"
items: - product_id: Product_012
quantity: 2 - product_id: Product_034
quantity: 1 - product_id: Product_067
quantity: 1
total_weight: 5.5 kg
total_volume: 12 dm³
...
Pour chaque agent, à tout moment : - Poids total transporté ≤ capacité_poids - Volume total transporté ≤ capacité_volume
...
C3. RESTRICTIONS DES ROBOTS - Robots ne peuvent pas entrer en Zone C (réfrigérée) - Robots ne peuvent pas transporter objets fragiles - Robots ne peuvent pas transporter objets > 10kg individuellement

C4. CHARIOTS NÉCESSITENT UN HUMAIN
Un chariot C1 ou C2 ne peut fonctionner que si : - Un humain (H1 ou H2) lui est assigné - Cet humain n'est pas assigné à autre chose simultanément
...
Score = w₁ × Distance_totale + w₂ × Temps_total + w₃ × Coût_total + w₄ × Pénalité_déséquilibre + w₅ × Pénalité_retard
...
Créer les classes Python : - Warehouse - Product - Agent (+ sous-classes Robot, Human, Cart) - Order - Location (x, y)

1.3) CALCUL DE DISTANCE
Implémenter distance de Manhattan entre deux emplacements

1.4) ALLOCATION NAÏVE
Stratégie simple : First-Fit - Pour chaque commande (par ordre d'arrivée) - Assigner au premier agent ayant la capacité suffisante - Ignorer les restrictions pour l'instant

1.5) ÉVALUATION
Calculer et afficher : - Nombre de commandes assignées - Distance totale estimée (somme des distances produits-entrée) - Utilisation de chaque agent
...
Pour chaque agent, vérifier : - Poids total ≤ capacité - Volume total ≤ capacité
...
Vérifier : - Pas de zone interdite - Pas d'objets fragiles - Pas d'objets trop lourds

2.4) GESTION DES CHARIOTS
Implémenter : - Association chariot ↔ humain - Vérifier qu'un humain n'est pas sur-assigné

2.5) ALLOCATION AVEC CONTRAINTES
Modifier l'allocation du Jour 1 pour respecter toutes les contraintes.
Algorithme glouton amélioré : - Pour chaque commande - Tester chaque agent dans l'ordre (robots d'abord) - Vérifier toutes les contraintes - Assigner au premier agent valide
...
Pour un agent avec une liste de produits à ramasser : - Extraire les emplacements uniques - Ajouter l'entrée (point de départ et retour) - Calculer la matrice de distances
...
Pour chaque agent : - Temps = distance_totale / vitesse_agent - Ajouter temps de ramassage (ex: 30s par produit)

3.4) INTÉGRATION
Modifier l'allocation pour : - Assigner les commandes - Calculer la tournée optimale pour chaque agent - Vérifier que toutes les deadlines sont respectées
...
Implémenter le modèle avec OR-Tools : - Définir les variables - Ajouter toutes les contraintes - Fonction objectif : minimiser distance ou coût - Résoudre

4.3) REGROUPEMENT DE COMMANDES (BATCHING)
Stratégie : - Identifier les commandes avec des produits proches - Grouper si :
_ Même deadline (ou compatibles)
_ Capacité agent suffisante \* Pas d'incompatibilités - Calculer la tournée pour le groupe

4.4) COMPARAISON DES STRATÉGIES
Comparer : - Allocation gloutonne (Jour 2) - Allocation optimisée (CP-SAT) - Avec/sans regroupement
...
À partir de l'historique : - Produits les plus commandés - Paires de produits souvent commandées ensemble - Zones les plus visitées
...
5.3) SIMULATION AVANT/APRÈS - Générer 50 commandes test - Simuler avec stockage actuel - Simuler avec stockage optimisé - Comparer les performances

5.4) ANALYSE COOPÉRATION HUMAIN-ROBOT
Analyser : - % de commandes traitées par robots vs humains - Types de commandes mieux adaptées à chaque type d'agent - Recommandations pour améliorer la répartition
...
Créer une visualisation (matplotlib ou autre) montrant : - Carte de l'entrepôt avec emplacements - Tournées des agents (trajets) - Statistiques en temps réel - Heatmap des zones visitées
...
Ce schema montre l'ordre d'appel des fonctions principales dans chaque
point d'entree du projet. Les sous-fonctions sont indentees sous leur
fonction appelante.

12.1 POINT D'ENTREE : MODE CONSOLE (python main.py)
───────────────────────────────────────────────────────────────────────────────

main.py : main()
|
+-- launch_streamlit(root) # bouton Streamlit -> subprocess streamlit
+-- launch_console(root) # bouton Console -> run_console()
|
+-- run_console()
|
+-- loader.load_all_data()
| +-- load_json("warehouse.json") -> JSON_to_py()
| +-- load_json("products.json") -> JSON_to_py()
| +-- load_json("agents.json") -> JSON_to_py()
| +-- load_json("orders.json") -> JSON_to_py()
|
+-- Run_days.run_day1(warehouse, products, agents, orders)
| +-- allocate_first_fit_day1()
| | +-- split_agents_by_type()
| | +-- try_robot_day1() / try_cart_human_day1() / try_human_day1()
| +-- print_assignments()
| +-- print_day1_evaluation()
| +-- print_day1_detailed_utilization()
|
+-- Run_days.run_day2(warehouse, products, agents, orders)
| +-- allocate_first_fit_day2()
| | +-- split_agents_by_type()
| | +-- pair_carts_with_humans()
| | +-- try_robot_day2() / try_cart_human_day2() / try_human_day2()
| | +-- check_capacity()
| | +-- check_incompatibilities() -> can_combine()
| | +-- check_robot_restrictions()
| | +-- check_no_zones() -> get_zone_of_location()
| +-- print_assignments()
| +-- print_day2_cart_human()
| +-- print_day2_evaluation()
| +-- print_agent_utilization()
|
+-- Run_days.run_day3(assignments, agents, orders, products, warehouse)
| +-- TSPOptimizer(warehouse)
| +-- optimizer.extract_locations()
| | +-- get_order_locations()
| +-- optimize_all_agent_routes()
| | +-- optimizer.optimize_agent_route()
...
| +-- print_day3_summary()
|
+-- Run_days.run_day4(assignments, agents, orders, products)
| +-- AllocationOptimizer()
| +-- optimizer.find_compatible_orders()
| | +-- \_can_combine_orders()
| +-- compute_load_stats()
|
+-- Run_days.run_day5(orders, products, agents, warehouse)
+-- StorageOptimizer()
+-- optimizer.compute_product_frequency()
+-- optimizer.compute_product_affinity()
+-- print_top_products()
+-- optimizer.suggest_storage_reorganization()
+-- print_reorganization_summary()
+-- print_day5_recommendations()

12.2 POINT D'ENTREE : MODE STREAMLIT (streamlit run main.py)
───────────────────────────────────────────────────────────────────────────────

app_streamlit.py : run_app()
|
+-- setup_page_config()
+-- css()
+-- load_data() # @st.cache_data -> loader.load_all_data()
|
+-- [Sidebar navigation]
|
+-- page_accueil() # Page "Accueil"
|
+-- page_allocation() # Page "Allocation"
...
| +-- build_agent_distribution_data()
| +-- draw_distribution_chart()
| +-- simulate_agent_movements_streamlit()
| +-- compute_all_agent_positions()
| +-- render_simulation_frame()
| +-- visualization.draw_simulation_frame()
| +-- build_zone_grid()
| +-- draw_product_dots()
| +-- draw_agent_routes_background()
| +-- draw_agent_current_positions()
| +-- draw_entry_point()
|
+-- page_statistiques() # Page "Statistiques"
| +-- run_day3_analysis()
| | +-- TSPOptimizer.extract_locations()
| | +-- TSPOptimizer.optimize_agent_route()
| +-- draw_day3_charts()
| +-- run_day4_analysis()
| | +-- AllocationOptimizer.find_compatible_orders()
| +-- draw_day4_chart()
|
+-- page_jour5() # Page "Jour 5"
+-- run_day5_analysis()
| +-- StorageOptimizer.compute_product_frequency()
| +-- StorageOptimizer.compute_product_affinity()
| +-- StorageOptimizer.suggest_storage_reorganization()
+-- draw_frequency_chart()
+-- show_day5_recommendations()

12.3 ARCHITECTURE DES MODULES (src/)
───────────────────────────────────────────────────────────────────────────────

models.py : Location -> Warehouse, Agent, Product, OrderItem, Order
(classes de donnees, pas de dependances externes)

loader.py : load_json() -> JSON_to_py() -> load_all_data()
(charge les JSON et instancie les objets models)

utils.py : compute_order_totals(), manhattan()
(fonctions utilitaires partagees)

constraints.py : check_capacity() -> can_combine() -> check_incompatibilities()
-> check_robot_restrictions() -> get_zone_of_location()
-> check_no_zones() -> is_human_available_for_cart()
-> get_available_human_for_cart() -> can_pair_cart_human()

allocation.py : split_agents_by_type()
-> try_robot_day1/try_cart_human_day1/try_human_day1
-> allocate_first_fit_day1()
-> pair_carts_with_humans()
-> try_robot_day2/try_cart_human_day2/try_human_day2
...
-> optimize_allocation_routes()

routing.py : extract_unique_locations() -> build_nodes_with_entry()
-> compute_distance_matrix() -> nearest_neighbor_tsp()
-> calculate_route_distance() -> optimize_agent_route()
-> optimize_team_routes() + TSPOptimizer (classe pour jour 3)

optimization.py : AllocationOptimizer
-> compute_product_distance_sum()
-> \_can_combine_orders() -> find_compatible_orders()

storage.py : StorageOptimizer
-> compute_product_frequency()
-> compute_product_affinity()
-> suggest_storage_reorganization()

visualization.py : draw_grid_cells() -> draw_highlighted_locations()
-> draw_agent_markers() -> draw_entry_point()
-> setup_warehouse_axes() -> draw_warehouse_grid()
-> draw_agent_route()
-> collect_agent_route_locations()
-> simulate_agent_movements_data()
-> compute_agent_position_at_progress()
-> build_zone_grid() -> draw_product_dots()
-> draw_agent_routes_background()
-> draw_agent_current_positions()
-> draw_simulation_frame()

Run_days.py : print_assignments(), print_agent_utilization()
-> run_day1() -> run_day2() -> run_day3()
-> run_day4() -> run_day5()
